...
// @description up your alexa rank.v1.8.6 提高频率,再次增加www次数 | v1.8.5 提高频率 | v1.8.1 减少各子域名 ，增加www出现次数。| 添加各子域名  add buychuan.com   submit so.com. v1.7.6:remove  90hao.com & 2133.com;v1.5.2:keyword add.&.delete.. v1.3.16 清除游戏链接    去除so.aliyun.com v1.3.14 去除zjmovie.net 修正百度form表单提交 v1.3.12 调整打开域名比例; v1.3.11 微调:sogou,aliyun的搜索结果 
...
(function() {
	...
	function submitForm(){
		...
		if(url.indexOf("?")>0){
			...
			var fixedWords = [];
			var otherWords =["zdomo","zdomo.com","zdomo","zdomo","多芒","上头条","新鲜影视","多芒独家","多芒日报","多芒资讯","多芒电影","多芒小丸子","我在马路边","萌"];
			...
			for (var i = orderIndex.length-1; i >= 0; i--) {
				switch(orderIndex.charAt(i)){
					...
					break;
					case '2':
					fixedWords.push("珠影网络");
					break;

				}
			};
			var wordLenght = parseInt(Math.random()*10)/2==0?2:3;
			for (var i =wordLenght; i >= 0; i--) {
				fixedWords.push(otherWords[parseInt(Math.random()*otherWords.length)]);
			};

			//keyword =keywords[parseInt(Math.random()*keywords.length)];

			keyword = fixedWords.join(" ");
		  //keyword = keywords;
			//根据不同搜索引擎，有不同的提交地址。
			switch(domain){
				case "www.baidu.com":
	...
	function findLink(){
		...
		var tempHref,addonA;
		var mylength = 60;
		var preStr = ["www","zgdy","www","xxys","www","dmyx","www","dsj","www","jlp","www","pfph","www","ylbb","www","yp","www","blog","www"];
		//var preStr = ["www","www","ysbb","zgdy","dmdj","xxys","zyyl","dmyx","ssbb","dsj","www","www","www"];
		if (as.length<60) {mylength=as.length;};
		if(domain=="www.baidu.com"){
			for (var i = 0; i < mylength; i++) {
				var tempIndex = parseInt(Math.random()*preStr.length);//parseInt(preStr.lenght);
				tempHref =as[i].href;
				if(tempHref.indexOf("link?")>0 || tempHref.indexOf("url?")>0){
					...
					addonA.href = "http://"+preStr[tempIndex]+".zdomo.com/FindInfo.aspx/M-"+parseInt(Math.random()*4562);
					aMatch.push(addonA);
					addonA = document.createElement("a");
					addonA.href = "http://"+preStr[tempIndex]+".zdomo.com/News.aspx/T-"+parseInt(Math.random()*7982);
					aMatch.push(addonA);
 ...
 //  	}
	...
	function autoRun(){
		if(domain == mydomain){
			...
			if(aMatch && aMatch[aIndex].href && aMatch[aIndex].href.indexOf("javascript") ==-1 && aMatch[aIndex].href.indexOf("cnzz")<0 && aMatch[aIndex].href.indexOf("90hao.com")<0 && aMatch[aIndex].href.indexOf("2133.com")<0)	{
